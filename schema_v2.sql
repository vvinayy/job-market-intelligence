-- =====================================================================
-- SCHEMA v2 — cities, states, cleaned_postings shape
--
-- Run order: schema.sql, THEN this file, then trends_setup.sql.
--
-- The cleaning logic that used to live here (clean_and_populate() and
-- its helper functions, plus the city_aliases/skill_aliases/role_patterns
-- lookup tables that only those functions used) has moved to cleaning.py.
-- This file now only defines tables: raw data in, cleaned data out —
-- the actual transformation happens in Python, not in Postgres.
--
-- Run once:
...
-- cleaned_postings IS rebuilt. Everything in it is reproducible from
-- raw_postings by re-running `python cleaning.py`.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- CITIES — the canonical list. Still queried directly by the API
-- (/reference/cities, /reference/states) and now also by cleaning.py,
-- which builds its own city-name -> city_id lookup from this table at
-- the start of each run rather than going through a SQL alias table.
--
...
--     rather than silent
--   * role_family: the classify_role() output, folded directly into
--     this table's shape rather than added by a later ALTER
-- ---------------------------------------------------------------------
